...
#else				
		case DL_UART_MAIN_IIDX_DMA_DONE_RX:				// ANODMA
        {
//					wit_Getdata();											  		  
//					memset(ReadBuffWit, '\0', 44);
					
          ANO_Flag=1;//ANO接收标志位
					
					DL_DMA_setSrcAddr(DMA, DMA_CH1_CHAN_ID, (uint32_t)(&UART1->RXDATA));
					DL_DMA_setDestAddr(DMA, DMA_CH1_CHAN_ID, (uint32_t) &ReadBuffAno[0]);
					DL_DMA_setTransferSize(DMA, DMA_CH1_CHAN_ID, ANOSIZE);//46个字节
...
//		case DL_UART_MAIN_IIDX_DMA_DONE_RX:				// 维特陀螺仪DMA UART1_DMA CH1 
//        {
//					wit_Getdata();											  		  
//					memset(ReadBuffWit, '\0', 44);
//					DL_DMA_setSrcAddr(DMA, DMA_CH0_CHAN_ID, (uint32_t)(&UART1->RXDATA));
...
#else				
		case DL_UART_MAIN_IIDX_DMA_DONE_RX:				// 维特陀螺仪DMA
        {
					wit_Getdata();											  		  
					memset(ReadBuffWit, '\0', 44);
...
void GROUP1_IRQHandler (void)
{
    ...
    else
    {
		...
		NVIC_ClearPendingIRQ(GPIOB_INT_IRQn);	// 留意
		NVIC_ClearPendingIRQ(GPIOA_INT_IRQn);	// 留意

...
int16 imu963ra_gyro_x_iic = 0, imu963ra_gyro_y_iic = 0, imu963ra_gyro_z_iic = 0;
int16 imu963ra_acc_x_iic = 0,  imu963ra_acc_y_iic = 0,  imu963ra_acc_z_iic = 0;
int16 imu963ra_mag_x_iic = 0,  imu963ra_mag_y_iic = 0,  imu963ra_mag_z_iic = 0;
...
//// 参数说明     data            数据
//// 返回参数     void
//// 使用示例     imu963ra_write_acc_gyro_register_iic(IMU963RA_SLV0_CONFIG, 0x00);
//// 备注信息     内部调用
////-------------------------------------------------------------------------------------------------------------------
//#define imu963ra_write_acc_gyro_register_iic(reg,data)       (soft_iic_write_8bit_register(&imu963ra_iic_struct,reg,data))
...
//// 备注信息     内部调用
////-------------------------------------------------------------------------------------------------------------------
//static void imu963ra_write_acc_gyro_register_iic (uint8 reg, uint8 data)
//{
//    IMU963RA_CS(0);
...
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
#define imu963ra_read_acc_gyro_register_iic(reg)             (soft_iic_sccb_read_register(&imu963ra_iic_struct,reg))
...
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
#define imu963ra_read_acc_gyro_registers_iic(reg,data,len)   (soft_iic_read_8bit_registers(&imu963ra_iic_struct,reg,data,len))
...
static uint8 imu963ra_write_mag_register (uint8 addr, uint8 reg, uint8 data)
{
    ...
    addr = addr << 1;
    imu963ra_write_acc_gyro_register_iic(IMU963RA_SLV0_CONFIG, 0x00);               // 从机0配置清除
    imu963ra_write_acc_gyro_register_iic(IMU963RA_SLV0_ADD, addr | 0);              // 设置地磁计地址（注意这里需要设置8位的I2C地址） 0x2C
    imu963ra_write_acc_gyro_register_iic(IMU963RA_SLV0_SUBADD, reg);                // 需要写入的寄存器地址
    imu963ra_write_acc_gyro_register_iic(IMU963RA_DATAWRITE_SLV0, data);            // 需要写入的数据
    imu963ra_write_acc_gyro_register_iic(IMU963RA_MASTER_CONFIG, 0x4C);             // 仅在第一个周期启用通讯 开启上拉 I2C主机使能
    
    // 等待通讯成功
    while(0 == (0x80 & imu963ra_read_acc_gyro_register_iic(IMU963RA_STATUS_MASTER)))
    {
...
static uint8 imu963ra_read_mag_register (uint8 addr, uint8 reg)
{
    ...
    addr = addr << 1;
    imu963ra_write_acc_gyro_register_iic(IMU963RA_SLV0_ADD, addr | 1);              // 设置地磁计地址（注意这里需要设置8位的I2C地址） 0x2C
    imu963ra_write_acc_gyro_register_iic(IMU963RA_SLV0_SUBADD, reg);                // 需要读取的寄存器地址
    imu963ra_write_acc_gyro_register_iic(IMU963RA_SLV0_CONFIG, 0x01);    
    imu963ra_write_acc_gyro_register_iic(IMU963RA_MASTER_CONFIG, 0x4C);             // 仅在第一个周期启用通讯 开启上拉 I2C主机使能
    
    // 等待通讯成功
    while(0 == (0x01 & imu963ra_read_acc_gyro_register_iic(IMU963RA_STATUS_MASTER)))
    {
        if(IMU963RA_TIMEOUT_COUNT < timeout_count ++)
        {
            break;
    ...
    return (imu963ra_read_acc_gyro_register_iic(IMU963RA_SENSOR_HUB_1));            // 返回读取到的数据
}
...
static void imu963ra_connect_mag (uint8 addr, uint8 reg)
{
    ...
    imu963ra_write_acc_gyro_register_iic(IMU963RA_SLV0_ADD, addr | 1);              // 设置地磁计地址（注意这里需要设置8位的I2C地址） 0x2C
    imu963ra_write_acc_gyro_register_iic(IMU963RA_SLV0_SUBADD, reg);                // 需要读取的寄存器地址
    imu963ra_write_acc_gyro_register_iic(IMU963RA_SLV0_CONFIG, 0x06);    
    imu963ra_write_acc_gyro_register_iic(IMU963RA_MASTER_CONFIG, 0x6C);             // 仅在第一个周期启用通讯 开启上拉 I2C主机使能
}   
...
static uint8 imu963ra_acc_gyro_self_check (void)
{
    ...
    while(0x6B != dat)                                                          // 判断 ID 是否正确
    {
        ...
        dat = imu963ra_read_acc_gyro_register_iic(IMU963RA_WHO_AM_I);
        system_delay_ms(10);
    }
    return return_state;
}
...
//-------------------------------------------------------------------------------------------------------------------
void imu963ra_get_acc_iic (void)
{
    uint8 dat[6];

    imu963ra_read_acc_gyro_registers_iic(IMU963RA_OUTX_L_A, dat, 6);
    imu963ra_acc_x_iic = (int16)(((uint16)dat[1] << 8 | dat[0]));
    imu963ra_acc_y_iic = (int16)(((uint16)dat[3] << 8 | dat[2]));
    imu963ra_acc_z_iic = (int16)(((uint16)dat[5] << 8 | dat[4]));
}
...
// 备注信息     执行该函数后，直接查看对应的变量即可
//-------------------------------------------------------------------------------------------------------------------
void imu963ra_get_gyro_iic (void)
{
    uint8 dat[6];

    imu963ra_read_acc_gyro_registers_iic(IMU963RA_OUTX_L_G, dat, 6);
    imu963ra_gyro_x_iic = (int16)(((uint16)dat[1] << 8 | dat[0]));
    imu963ra_gyro_y_iic = (int16)(((uint16)dat[3] << 8 | dat[2]));
    imu963ra_gyro_z_iic = (int16)(((uint16)dat[5] << 8 | dat[4]));
}
...
// 备注信息     执行该函数后，直接查看对应的变量即可
//-------------------------------------------------------------------------------------------------------------------
void imu963ra_get_mag_iic (void)
{
    uint8 temp_status;
    uint8 dat[6];

    imu963ra_write_acc_gyro_register_iic(IMU963RA_FUNC_CFG_ACCESS, 0x40);
    temp_status = imu963ra_read_acc_gyro_register_iic(IMU963RA_STATUS_MASTER);
    if(0x01 & temp_status)
    {
        imu963ra_read_acc_gyro_registers_iic(IMU963RA_SENSOR_HUB_1, dat, 6);
        imu963ra_mag_x_iic = (int16)(((uint16)dat[1] << 8 | dat[0]));
        imu963ra_mag_y_iic = (int16)(((uint16)dat[3] << 8 | dat[2]));
        imu963ra_mag_z_iic = (int16)(((uint16)dat[5] << 8 | dat[4]));
    }
    imu963ra_write_acc_gyro_register_iic(IMU963RA_FUNC_CFG_ACCESS, 0x00);
}
...
//#endif
   ...
   soft_iic_init(&imu963ra_iic_struct, IMU963RA_DEV_ADDR, IMU963RA_SOFT_IIC_DELAY, IMU963RA_SCL_PIN, IMU963RA_SDA_PIN);
    do
    {
        imu963ra_write_acc_gyro_register_iic(IMU963RA_FUNC_CFG_ACCESS, 0x00);       // 关闭HUB寄存器访问
        imu963ra_write_acc_gyro_register_iic(IMU963RA_CTRL3_C, 0x01);               // 复位设备
        system_delay_ms(2);                             
        imu963ra_write_acc_gyro_register_iic(IMU963RA_FUNC_CFG_ACCESS, 0x00);       // 关闭HUB寄存器访问
        ...
        imu963ra_write_acc_gyro_register_iic(IMU963RA_INT1_CTRL, 0x03);             // 开启陀螺仪 加速度数据就绪中断
        ...
        switch(IMU963RA_ACC_SAMPLE_DEFAULT)
        {
            ...
            {
                imu963ra_write_acc_gyro_register_iic(IMU963RA_CTRL1_XL, 0x30);
                imu963ra_transition_factor[0] = 16393;
            }break;
            case IMU963RA_ACC_SAMPLE_SGN_4G:
            {
                imu963ra_write_acc_gyro_register_iic(IMU963RA_CTRL1_XL, 0x38);
                imu963ra_transition_factor[0] = 8197;
            }break;
            case IMU963RA_ACC_SAMPLE_SGN_8G:
            {
                imu963ra_write_acc_gyro_register_iic(IMU963RA_CTRL1_XL, 0x3C);
                imu963ra_transition_factor[0] = 4098;
            }break;
            case IMU963RA_ACC_SAMPLE_SGN_16G:
            {
                imu963ra_write_acc_gyro_register_iic(IMU963RA_CTRL1_XL, 0x34);
        ...
        imu963ra_write_acc_gyro_register_iic(IMU963RA_CTRL3_C, 0x44);               // 使能陀螺仪数字低通滤波器
        imu963ra_write_acc_gyro_register_iic(IMU963RA_CTRL4_C, 0x02);               // 使能数字低通滤波器
        imu963ra_write_acc_gyro_register_iic(IMU963RA_CTRL5_C, 0x00);               // 加速度计与陀螺仪四舍五入
        imu963ra_write_acc_gyro_register_iic(IMU963RA_CTRL6_C, 0x00);               // 开启加速度计高性能模式 陀螺仪低通滤波 133hz
        imu963ra_write_acc_gyro_register_iic(IMU963RA_CTRL7_G, 0x00);               // 开启陀螺仪高性能模式 关闭高通滤波
        imu963ra_write_acc_gyro_register_iic(IMU963RA_CTRL9_XL, 0x01);              // 关闭I3C接口

        imu963ra_write_acc_gyro_register_iic(IMU963RA_FUNC_CFG_ACCESS, 0x40);       // 开启HUB寄存器访问 用于配置地磁计
        imu963ra_write_acc_gyro_register_iic(IMU963RA_MASTER_CONFIG, 0x80);         // 复位I2C主机
        system_delay_ms(2);                             
        imu963ra_write_acc_gyro_register_iic(IMU963RA_MASTER_CONFIG, 0x00);         // 清除复位标志
        system_delay_ms(2);

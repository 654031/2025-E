...
high_res = 320               # 高分辨率模式宽高,越高越清晰但是帧率越低，注意 std_res 也要跟着改大点
model_path = "/root/models/model_3356.mud" # 检测黑框模型路径
...
find_laser = False           # 找激光点，实际使用时直接把摄像头中心和激光点保持移植就好了，不需要找激光点
...
if not os.path.exists(model_path):
    model_path1 = "root/models/model_3356.mud"
...
class ano_transfer:
    ...
    class ano_frameStruct:
        head = 0xAA                 #<帧头
        target_addr = 0xFF          #<目标地址
        function_id = 0xFF          #<该帧要实现某功能的功能码id
        data_len = 0                #<数据长度
        data = bytearray()          #<数据内容，协议最高只支持*字节数据
        sum_check = bytes(0)        #<和校验
        add_check = bytes(0)        #<附加校验


        def ano_set_address(self, d_addr):
            self.target_addr = d_addr


        def ano_set_type(self, ident):
            self.function_id = ident


    def __init__(self):
        self.recv_frame = self.ano_frameStruct()
        self.send_frame = self.ano_frameStruct()

        device = "/dev/ttyS0"
        self.uart = uart.UART(device,115200)


    @staticmethod
    def frame_turn_to_array(frame):
        byte_frame = bytearray()
        byte_frame += frame.head.to_bytes(1,'little')
        byte_frame += frame.target_addr.to_bytes(1,'little')
        byte_frame += frame.function_id.to_bytes(1,'little')
        byte_frame += frame.data_len.to_bytes(1,'little')
        byte_frame += frame.data
        byte_frame += frame.sum_check
        byte_frame += frame.add_check
        return byte_frame


    @staticmethod
    def ano_check_calculate(frame):
        byte_frame = bytearray()
        byte_frame += frame.head.to_bytes(1,'little')
        byte_frame += frame.target_addr.to_bytes(1,'little')
        byte_frame += frame.function_id.to_bytes(1,'little')
        ...
        sum_int = int(sum_int % 256)
        add_int = int(add_int % 256)
        sum_byte = sum_int.to_bytes(1,'little')
        add_byte = add_int.to_bytes(1,'little')
        return [sum_byte,add_byte]


    def ano_check(self):
        [sumb,addb] = self.ano_check_calculate(self.recv_frame)

        if sumb != self.recv_frame.sum_check or addb != self.recv_frame.add_check:
            return False
        return True


    def ano_usart_send(self,sendbyte):
        self.uart.write(bytes(sendbyte))


    def ano_send_string(self,string):
        byte_str = string.encode('utf-8')
        self.send_frame.data = byte_str
        self.send_frame.data_len = len(byte_str)
        [sumb,addb] = self.ano_check_calculate(self.send_frame)
        self.send_frame.sum_check = sumb
        self.send_frame.add_check = addb

        self.ano_usart_send(self.frame_turn_to_array(self.send_frame))
    ...
    def ano_receivebybyte(self,byte):
        if byte == b'\xaa':
            self.RecvBuffer = bytearray()
            self.RecvBuffer += byte
            self.RecvIndex+=1
        else:
            if self.RecvIndex > 0:
                self.RecvBuffer += byte
                if self.RecvIndex < 3:
                    a=1
                elif self.RecvIndex == 3:
                    self.DataLength = int.from_bytes(byte,byteorder="little")

                elif self.RecvIndex >= self.DataLength + 5:
                    #处理
    ...
    def push_frame(self):
        if len(self.RecvBuffer) < 6:
            print('awmc')
            return
        self.recv_frame.head = self.RecvBuffer[0]
        self.recv_frame.target_addr = self.RecvBuffer[1]
        self.recv_frame.function_id = self.RecvBuffer[2]
        self.recv_frame.data_len = self.RecvBuffer[3]
        self.recv_frame.data = self.RecvBuffer[4:-2]
        print(self.recv_frame.data)
        self.recv_frame.sum_check = self.RecvBuffer[-2].to_bytes(1,"little")
        self.recv_frame.add_check = self.RecvBuffer[-1].to_bytes(1,"little")
        if not self.ano_check():
            #包损坏
            print('awmc')
        else:
            self.frame_process()
    ...
    def frame_process(self):
        Frame = [self.recv_frame.target_addr,self.recv_frame.function_id,self.recv_frame.data]
        self.RecvFrames.append(Frame)


    def is_available(self):
        data = bytes(self.uart.read(len = -1,timeout = 0))
        if len(data) == 0:
            return False
        else:
            for ch in data:
                self.ano_receivebybyte(ch.to_bytes(1,"little"))
        
        if len(self.RecvFrames) > 0:
            return True
        
        return False


    def ano_printf(self,format_str,*args):
        data_ = bytearray()
        index=0
        for ch in format_str:
            if ch == 'b':
                    #data_ += args[index].to_bytes(1,"little")
                    print('not supported')
            elif ch == 'w':
                    #data_ += args[index].to_bytes(2,"little")
                    print('not supported')
            elif ch == 'i':
                    data_ += args[index].to_bytes(4,"little")
            index+=1
        self.send_frame.data = data_
        self.send_frame.data_len = len(data_)
        [sumb,addb] = self.ano_check_calculate(self.send_frame)
        self.send_frame.sum_check = sumb
        self.send_frame.add_check = addb
    ...
    def get_frame(self):
        return self.RecvFrames.pop()

    
    def ano_scanf(self,format_str):
        data = self.get_frame()[2]
        index=0
        recv = []
        for ch in format_str:
            if ch == 'b':
                    ...
                    recv.append(int.from_bytes(data[index:index+4],'little'))
                    index+=4
        return recv
...
while not app.need_exit():
    ...
    if max_idx >= 0:
        ...
        if approx is not None:
            ...
            if woligedou:
                ...
                frame.ano_printf("iiii",fangxiang_x,piancha_x,fangxiang_y,piancha_y)
                print(last_center_small[0],",",last_center_small[1])
        if DEBUG:
    ...
    if DEBUG or debug_show_hires:
        ...
        img.draw_string(5, 5, f"FPS: {fps:.1f}", image.Color.from_rgb(255, 255, 255), scale=0.8)
        disp.show(img)
    else:
        if debug_draw_err_line:
        ...
        fps = 1000 / (time.ticks_ms() - t_start)
        img_ai.draw_string(5, 5, f"FPS: {fps:.1f}", image.Color.from_rgb(255, 255, 255), scale=0.8)
        disp.show(img_ai)
